...
// Pass Bun `SQL.Options`: URL, `adapter`, or SQLite `filename`. UQL infers the dialect.
const pool = new BunSqlQuerierPool(
  process.env.DATABASE_URL!, // e.g. postgres://, mysql://, file path, or :memory:
  { entities: [User, Post] },
);
...
```ts
const pool = new BunSqlQuerierPool(':memory:', { entities: [User] });
```
