...
- **UQL's Role**: Use a Cockroach connection URL (or `adapter: 'cockroachdb'` in `SQL.Options` when Bun exposes it). UQL keeps `SqlDialect` as `cockroachdb` for AST generation while `normalizeBunOpts` maps Bun's `adapter` to `postgres` under the hood.
