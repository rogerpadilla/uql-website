...
## Community Updates (Mar 18, 2026) + quick changelog

- Added Joist as a full participant across all comparison sections (not just matrix entries).
...
:::note
This measures **SQL generation overhead**—the pure CPU time spent by the ORM before hitting the database. UQL achieves this via **zero-allocation architecture** and pre-computed schema lookups at startup. The benchmark is [fully open-source](https://github.com/rogerpadilla/ts-orm-benchmark).
:::
