...
In Prisma, TypeORM, and Sequelize, a plain query grabs its own pooled connection, so `Promise.all` parallelizes. UQL's [pool reads](/querying/querier#parallel-reads-on-the-pool) work the same way - `pool.findMany(User, {...})` acquires, runs, and releases per call. Reach for `pool.withQuerier` / `pool.transaction` when statements must share one connection or run atomically.
