...
### Combine metadata filters with vector ranking

Restrict the candidate set with a normal `$where` filter before ranking by similarity; the filter and the vector sort run in a single query.
